...
const Testimonial2 = () => {
  ...
  return (
    <Slider {...settings} className=" px-8  md:mx-auto mt-16 mb-16 max-w-2xl text-slate-950 ">
    {testimonialsData.map(testimonial => (
      <div key={testimonial.id} className=" px-6 py-6 bg-teal-100 shadow-lg relative left-0 top-0 rounded-md ">
        <img src={testimonial.avatar} alt={testimonial.name} className="rounded-full  mb-2 w-20 shadow-lg static " />
        <div className='flex flex-col gap-1 '>
        <h3 className="text-lg font-bold">{testimonial.name}</h3>
        <p className="text-sm text-slate-700">{testimonial.email}</p>
        <p className="text-base mt-2">{testimonial.feedback}</p>
        <p className="text-xl font-semibold  text-slate-700">{testimonial.date}</p>
        </div>
      </div>
    ))}
  </Slider>
